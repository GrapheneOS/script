...
branch=pie-b4s4
aosp_version=PQ3B.190801.002
aosp_version_real=PQ3B.190801.002
aosp_tag=android-9.0.0_r47
aosp_tag_main=android-9.0.0_r46
...
declare -A kernels=(
  [google_marlin]=android-9.0.0_r0.111 # August 2019
  [google_wahoo]=android-9.0.0_r0.112 # August 2019
  [google_crosshatch]=android-9.0.0_r0.113 # August 2019
  [google_bonito]=android-9.0.0_r0.114 # August 2019
...
for repo in "${aosp_forks[@]}"; do
  ...
  if [[ -n $build_number ]]; then
    ...
    if [[ $repo != platform_manifest ]]; then
        ...
        git cherry-pick --ff $aosp_tag_main..pie
    fi
    git push -f || exit 1
  fi
