...
for repo in "${aosp_forks[@]}"; do
    ...
    if [[ $action == delete ]]; then
        ...
        if [[ $repo != @(device_google_gs101|device_google_gs201|device_google_shusky|device_google_zuma|device_google_zuma-sepolicy|platform_build|platform_external_android-nn-driver|platform_external_armnn|platform_frameworks_base|platform_manifest) ]]; then
