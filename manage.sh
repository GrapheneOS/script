...
for repo in "${aosp_forks[@]}"; do
    ...
    if [[ -n $build_number ]]; then
        ...
        if [[ $repo == @(device_google_gs201|device_google_pantah|platform_bionic|platform_manifest) ]]; then
...
for repo in ${independent[@]}; do
    ...
    if [[ -n $build_number ]]; then
        if [[ $repo == @(kernel_manifest-bluejay|kernel_manifest-coral|kernel_manifest-pantah|kernel_manifest-redbull|kernel_manifest-raviole) ]]; then
            git checkout -B tmp
            sed -i s%refs/heads/$branch_base%refs/tags/$aosp_version.$build_number% default.xml
            git commit default.xml -m $aosp_version.$build_number
            git push -fu origin tmp
        else
            git tag -s $aosp_version.$build_number -m $aosp_version.$build_number
        ...
        if [[ $repo == @(device_google_pantah-kernel|script) ]]; then
            git push -f
        else
